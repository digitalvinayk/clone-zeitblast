...
export async function GET(request: Request) {
  try {
    ...
    const contactsByStatusMap = contactsByStatus.reduce((acc: { [key: string]: number }, item) => {
    ...
    const messagesByStatusMap = messagesByStatus.reduce((acc: { [key: string]: number }, item) => {
      acc[item.status] = item._count;
      return acc;
    }, {} as { [key: string]: number });
